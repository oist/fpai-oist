...
/**
 * This class is a modification of {@link SocketAppender}. The queue type and the dispatch method is different than the
 * original version. <br/>
 * The connection thread is going to be closed only if an event with the marker
 * {@link ClassicConstants.FINALIZE_SESSION_MARKER} is sent. <br/>
 * <br/>
 * For example:<br/>
...
public class LogstashTcpSocketAppender extends AppenderBase<ILoggingEvent>
    ...
    /**
     * Default size of the queue used to hold logging events that are destined for the remote peer. Assuming an average
     * log entry to take 1k, this would result in the application using about 10MB additional memory if the queue is
     * full
     */
    ...
    /**
     * Get the pre-serialization transformer that will be used to transform each event into a Serializable object before
     * delivery to the remote receiver.
     *
     ...
     * <p>
     * The default implementation creates an instance of {@link DefaultSocketConnector}. A subclass may override to
     * provide a different {@link SocketConnector} implementation.
     *
     * @param address
    ...
    /**
     * The <b>Port</b> property takes a positive integer representing the port where the server is waiting for
     * connections.
     */
    ...
    /**
     * The <b>reconnectionDelay</b> property takes a positive {@link Duration} value representing the time to wait
     * between each failed connection attempt to the server. The default value of this option is to 30 seconds.
     *
     * <p>
    ...
    /**
     * The <b>eventDelayLimit</b> takes a non-negative integer representing the number of milliseconds to allow the
     * appender to block if the underlying BlockingQueue is full. Once this limit is reached, the event is dropped.
     *
    ...
    /**
     * Sets the maximum number of entries in the queue. Once the queue is full additional entries will be dropped if in
     * the time given by the <b>eventDelayLimit</b> no space becomes available.
     *
     * @param queueSize
